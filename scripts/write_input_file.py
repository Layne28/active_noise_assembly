...
def main():
    ...
    with open(args.indir + '/%s_active_noise_assembly_kT=%.01f_phi=%.02f_va=%.02f_tau=%.02f_lambda=%.02f_Lx=%.01f_Ly=%.01f_nx=%d_ny=%d.in' % (args.potential_type, float(args.kT), float(args.phi), float(args.va),float(args.tau),float(args.Lambda),float(args.Lx),float(args.Ly), int(args.nx), int(args.ny)), 'w') as f:
